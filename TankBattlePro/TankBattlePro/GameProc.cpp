...
void DrawScene(HDC hdc, LEVEL curL) {
	...
	case LEVEL2_OPEN:
		cb = GetSuccPic();
		wsprintf(str, L"%s", L"请按下回车键开始下一关卡" );
		break;
	case SUCCEED:
		cb = GetSuccPic();
		wsprintf(str, L"%s", L"胜利!请按下回车键重新开始游戏");
		break;
	case FAIL:
		cb = GetFailPic();
		wsprintf(str, L"%s", L"Fail!请按下回车键重新开始");
		break;
	}
...
void Destory() {
	Destory(pHeadBlocks);
	Destory(pHeadEnemys);
	Destory(pHeadBullets);
	Destory(pHeadEnemyBullets);
	free(pPlayer);
	free(pHeadBlocks);
	free(pHeadBullets);
...
void NextFrame(int ts) {///////////////////////////////////////////////////////////////////////
	NextFrameEntities(pHeadEnemys, ts);
	NextFrameEntities(pHeadBullets, ts);
	NextFrameEntities(pHeadEnemyBullets, ts);
...
void Update(int ts) {
	...
	for (Entity* p = pHeadEnemyBullets->next; p != NULL; p = p->next) {
		if (freeP != NULL) {
			Destory(pHeadEnemyBullets, freeP);
			freeP = NULL;
		}
		if (WallCollider(p, wndWidth, wndHeight))
			freeP = p;
