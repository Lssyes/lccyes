...
//		LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
	...
	tank1[0] = (HBITMAP)LoadImage(NULL, L"Resources\\TankBlue.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_DEFAULTSIZE | LR_LOADFROMFILE);
	...
	if(LEVEL1 == l) {
		block = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_OBSTACLE));
...
void InitEntity(Entity* headEnemy, Entity* headBlock, Entity* headBullet, Entity* headEnemyBullet, Entity* player, LEVEL l) {
	...
	Destory(headBlock);
	Destory(headBullet);
	Destory(headEnemyBullet);
	headEnemy->s = GetPrivateProfileInt(L"Enemy", L"size", 50, fileIni);
	headEnemy->dir = Dir(UP + rand() % 4);
	headEnemy->v = rand() % 2 == 0 ? GetPrivateProfileInt(L"Enemy", L"velf", 4, fileIni)
		: GetPrivateProfileInt(L"Enemy", L"vels", 2, fileIni);
